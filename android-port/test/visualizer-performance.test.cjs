...
test('position updates move the real SVG marker without rebuilding toolpaths', async () => {
    ...
    const compile=optimized=>{
        ...
        const built=buildSync({stdin:{contents:source,loader:'tsx',resolveDir:path.dirname(filename)},bundle:true,packages:'external',platform:'node',format:'cjs',jsx:'automatic',write:false});
    ...
    try {
        for(const optimized of [false,true]) {
            ...
            const toolpath=svg.querySelector('g path');const before=toolpath.getAttribute('d');
            ...
            assert.equal(toolpath.getAttribute('d'),before,'Toolpath geometry must be unchanged');
    ...
    } finally {GCodeSVGRenderer.prototype.rebuildToolpaths=original;cleanup();dom.window.close();delete globalThis.__visualizerPerfState;}
});

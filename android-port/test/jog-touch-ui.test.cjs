...
test('actual pendant jog button: quick release is immediate, hold jogs, cancellation clears the hold timer',async()=>{
 ...
 if(!process.env.TEST_ORIGINAL_JOG_TOUCH){source=require('../scripts/jog-touch-ui.cjs').transform(source,filename).code;source=require('../scripts/xy-pad-ui.cjs').transform(source,filename).code;}
 ...
 const releaseTiltZ=(direction)=>globalThis.__tiltZRelease(direction);const QUICK_PRESS_MS=110;const stopContinuousJog=()=>globalThis.__jogTouchStop();export ${component}`,{loader:'tsx',format:'cjs',jsx:'automatic'}).code;
 ...
 const calls=[];globalThis.__jogTouchStop=()=>calls.push('stop');globalThis.__tiltZRelease=()=>false;
 ...
 let view;await act(async()=>{view=render(React.createElement(Component,props));});let button=view.getByRole('button');
 ...
 try{
  ...
  globalThis.__tiltZRelease=direction=>{calls.push('release tilt Z '+direction);return true;};
  await act(async()=>view.rerender(React.createElement(Component,{...props,id:'z-plus',ariaLabel:'Jog Z plus',onShortPress:()=>calls.push('combined step'),onLongPress:()=>calls.push('combined hold')})));
  button=view.getByRole('button');at=calls.length;
  await act(async()=>{event('pointerdown');await wait(100);});
  await act(async()=>event('pointerup'));
  assert.deepEqual(calls.slice(at),['combined hold','release tilt Z 1'],'Z release must not call the upstream all-axis stop');
  at=calls.length;
  await act(async()=>{event('pointerdown');await wait(5);event('pointerup');});
  assert.deepEqual(calls.slice(at),['combined step'],'quick Z tap still uses a finite step');
  at=calls.length;await act(async()=>{event('pointerdown');await wait(100);event('pointercancel');});
  assert.deepEqual(calls.slice(at),['combined hold','release tilt Z 1']);

 }finally{cleanup();dom.window.close();delete globalThis.__jogTouchStop;delete globalThis.__tiltZRelease;}
});

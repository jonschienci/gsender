...
const preset={xyStep:.5,zStep:.1,feedrate:1000},boot='0123456789abcdef';
function rig(){
...
test('three sustained fast detents switch from exact steps to variable-speed short segments',()=>{
    ...
    assert.match(r.writes[2].line,/X0\.3000 F300/);
    r.ack();r.wheel(200,40);assert.match(r.writes.at(-1).line,/X0\.5000 F500/);
...
test('transition cancels an unfinished large STEP instead of buffering it behind velocity',()=>{
    const r=rig();r.wheel(0,0,'X',1,10000);r.ack();r.wheel(80,80,'X',1,10000);r.wheel(160,80,'X',1,10000);
    assert.equal(r.writes.length,1);assert.deepEqual(r.commands,['jog:stop']);assert.equal(r.m.queue.length,0);
    r.wheel(220,60,'X',1,10000);r.wheel(280,60,'X',1,10000);
    assert.equal(r.m.adaptive.phase,'fast');assert.equal(r.writes.length,2);
    assert.ok(Number(/X([\d.]+)/.exec(r.writes[1].line)[1])<1);
});
...
test('feed is capped by Precision AND the selected axis, and slows with wheel period',()=>{
    const r=rig();r.c.settings.settings.$110=100;r.fast();r.ack();
    assert.match(r.writes.at(-1).line,/F30\.000/);
    for(let i=0;i<6;i++){

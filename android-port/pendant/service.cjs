...
class Pendant {
    ...
    async connect() {
        ...
        try {
                ...
                try {
                    for (const line of this.lines.feed(bytes)) {
                        if (!this.gate) {
                            ...
                            if (event && this.armed) {
                                this.machine.submit(event, this.preset);
                            }
                        }
                    }
            ...
            this.openAt = this.now(); this.lastState = -Infinity;
    ...
    arm(body) {
        ...
        this.armed = true; this.machine.owned = true; this.reason = 'Precision jogging armed';
    }
    send(line) {
    ...
    tick() {
        ...
        try {
            ...
            if (this.armed && (now - this.uiAt >= 1500 || !this.gate.healthy())) this.disarm('UI/USB lease expired');
            this.machine.tick();
            if (now - this.lastPoll >= 100 && this.machine.current?.isOpen()) {
                this.lastPoll = now; this.machine.call('statusreport'); // Read-only, existing CNC connection.
            }
            if (now - this.lastState >= 100 && !this.pendingWrite) {
                this.lastState = now;
                const s = this.machine.snapshot();
                this.send(this.gate.state(s.xyz, s.valid, this.armed ? 'ARMED_' + s.state : 'DISARMED', this.armed));
            }
...
function start(options) {
    ...
    service.timer = setInterval(() => service.tick(), 20);
    service.timer.unref?.();
    return service;
}
function installRoutes(app) {
    ...
    app.get('/usb-pendant/:file', (req, res, next) => {
        if (!['launcher.js', 'panel.html', 'panel.js'].includes(req.params.file)) return next();

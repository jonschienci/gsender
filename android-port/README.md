...
Starting with Build 19, plug the SLB into USB OTG while gSender is open and it connects automatically. A board already attached when gSender starts also connects. Accept Android's USB permission prompt if shown. Automatic selection requires exactly one device with the SLB's STM32 CDC identity (`0483:5740`, first serial interface), an authenticated UI connection, and no active or pending board connection. Other serial controllers still use gSender's connection selector.

Selecting Disconnect keeps that attachment disconnected until it is unplugged and replugged; manual Connect remains available. A denied permission request or failed connection is attempted only once per attachment, avoiding repeated prompts. Replug to try automatically again, or use manual Connect. Autoconnect uses the existing gSender controller initialization and does not issue jog, homing, unlock, or job-start commands.
...
Build 19 passed 63 tests, Gradle assemble/lint, and APK verification. The packaged backend also passed startup connection, two hotplug reconnects, manual-disconnect suppression, knob exclusion, and no-motion-command checks on the Lenovo using Node 24.21.0 and a simulated USB transport. Physical SLB autoconnect testing remains to be done.

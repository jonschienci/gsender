...
export default function PendantShell() {
    ...
    useEffect(() => {
        ...
        const oldTheme = document.documentElement.dataset.darkTheme;
        document.documentElement.dataset.darkTheme = 'workshop';
        return () => {
            document.body.classList.remove('pendant-mode');
            if (oldTheme === undefined) delete document.documentElement.dataset.darkTheme;
            else document.documentElement.dataset.darkTheme = oldTheme;
        };
    }, []);

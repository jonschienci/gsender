...
export default function SpindlePanel({ mode }: Props) {
    ...
    const enabledSpindle =
        availableSpindles.find((s: any) => s.enabled) ?? null;
    const pendingSpindle =
        pendingSpindleIdRef.current !== null
            ? (availableSpindles.find(
                  (s: any) => s.id === pendingSpindleIdRef.current,
              ) ?? null)
            : null;
    const spindle =
        enabledSpindle ??
    ...
    const debounceSpindleSpeed = useCallback(
        debounce(
            (speed: number) => controller.command('spindlespeed:change', speed),
            300,
        ),
        [],
    );
    ...
    const getLaserOffsetCode = (
        ...
        const [xa, ya] = actions.calculateAdjustedOffsets(
            xOffset,
            yOffset,
            preferredUnits,
        );
        ...
        if (xOffset === 0 && yOffset !== 0)
            return [`G10 L20 P${currentWCS} Y${ya}`];
        if (xOffset !== 0 && yOffset === 0)
            return [`G10 L20 P${currentWCS} X${xa}`];
        if (xOffset !== 0 && yOffset !== 0)
            return [`G10 L20 P${currentWCS} X${xa} Y${ya}`];
        return [''];
    };

    // Full enableSpindleMode — ported verbatim from desktop (lines 352-406)
    const enableSpindleMode = () => {
        const preferredUnits: UNITS_GCODE | UNITS_EN =
    ...
    const actions = {
        ...
        handleLaserDurationChange: (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            setState((prev) => ({
                ...prev,
                laser: { ...prev.laser, duration: value },
            }));
        },
        runLaserTest: () => {
        ...
        calculateAdjustedOffsets(
            ...
            return [
                round(Number(x) + Number(xOffset), u),
                round(Number(y) + Number(yOffset), u),
            ];
        },
        getWCS(): number {
            return (
        ...
        handleHALSpindleSelect: (value: string) => {
            const selected = availableSpindles.find(
    ...
    const selectedOption =
        spindleOptions.find((o) => o.value === String(spindle.id)) ?? null;
            ...
            <div className="flex items-center gap-2 px-4 pb-3 shrink-0">
                ...
                >
                    <Select
                        ...
                        onChange={(opt) =>
                            opt && actions.handleHALSpindleSelect(opt.value)
                        }
                ...
                <div className="flex-[1] flex items-center justify-center gap-1">
                    ...
                    <button
                        ...
                        className={clsx(
                            ...
                            isLaserMode
                                ? 'bg-robin-500'
                                : 'bg-gray-300 dark:bg-surface-raised',

...
class GrblController {
	...
	constructor(engine, connection, options) {
		...
		this.runner.on("status", (res) => {
			if (
				!this.runner.hasSettings() &&
				res.activeState === GRBL_ACTIVE_STATE_IDLE
			) {
		...
		this.runner.on("parserstate", (res) => {
			...
			if (
				...
				this.runner.state.status.activeState === "Check"
			) {
		...
		const handler = {
			...
			"gcode:start": () => {
				...
				const [
					...
					zMax,
					safeHeight = 10,
					spindleDelay = defaultSpindleDelay,
				] = args;
				...
				if (lineToStartFrom && lineToStartFrom <= totalLines) {
					...
					if (hasSpindle) {
						modalGCode.push(
							`${modal.units} ${modal.spindle} F${feedRate} S${spindleRate}`,
						);
					} else {
	...
	write(data, context) {
		...
		this.actionMask.replyStatusReport =
			cmd === "?" || this.actionMask.replyStatusReport;
		this.actionMask.replyParserState =
			cmd === "$G" || this.actionMask.replyParserState;
	...
	setSenderTimeout() {
		...
		this.timeout = setTimeout(() => {
			...
			if (this.state.status.activeState === GRBL_ACTIVE_STATE_IDLE) {
				...
				this.state.status.activeState === GRBL_ACTIVE_STATE_IDLE &&

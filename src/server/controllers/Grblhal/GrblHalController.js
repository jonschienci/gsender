...
class GrblHalController {
	...
	constructor(engine, connection, options) {
		...
		this.sender = new Sender(SP_TYPE_CHAR_COUNTING, {
			...
			dataFilter: (line, context) => {
...

				{
					...
					if (programMode === "M0") {
						log.debug(
							`M0 Program Pause: line=${sent + 1}, sent=${sent}, received=${received}`,
						);
						...
						log.debug(
							`M1 Program Pause: line=${sent + 1}, sent=${sent}, received=${received}`,
						);
				...
				if (_.includes(words, "M6")) {
					log.debug(
						`M6 Tool Change: line=${sent + 1}, sent=${sent}, received=${received}`,
					);
		...
		this.sender.on("end", (finishTime) => {
			...
			if (
				this.runner.state.status.activeState === GRBL_HAL_ACTIVE_STATE_CHECK
			) {
		...
		this.runner.on("alarm", (res) => {
			...
			const alarm = this.settings?.alarms
				? this.settings.alarms[code.toString()]
				: _.find(GRBL_HAL_ALARMS, { code: code });
		...
		this.runner.on("parserstate", (res) => {
			...
			if (
				this.sender.state.finishTime > 0 &&
				this.sender.state.sent > 0 &&
				this.runner.state.status.activeState === GRBL_HAL_ACTIVE_STATE_CHECK
			) {
				this.workflow.stopTesting();
				this.command("gcode", "$C");
		...
		const queryStatusReport = () => {
			...
			if (this.isOpen()) {
				...
				if (this.runner.isAlarm() && this.actionMask.alarmCompleteReport) {
					...
					this.connection.writeImmediate(
						GRBLHAL_REALTIME_COMMANDS.STATUS_REPORT,
					); //? or \x80
				}
			}
		};
		...
		this.queryTimer = setInterval(() => {
			...
			if (this.state !== this.runner.state) {
				...
				if (
					this.workflow.isPaused() &&
					currentActiveState === GRBL_HAL_ACTIVE_STATE_HOLD &&
					(runnerActiveState === GRBL_HAL_ACTIVE_STATE_IDLE ||
						runnerActiveState === GRBL_HAL_ACTIVE_STATE_RUN)
				) {
					...
					this.programResumeTimeout = setTimeout(() => {
						...
						if (
							(as === GRBL_HAL_ACTIVE_STATE_IDLE ||
								as === GRBL_HAL_ACTIVE_STATE_RUN) &&
							this.sender.state.hold
						) {
		...
		const handler = {
			...
			"gcode:load": () => {
				...
				if (semver < ATCI_SUPPORTED_VERSION && spindleOnDelay > 0) {
					gcode = gcode.replace(
						/\b(?:S\d* ?M[34]|M[34] ?S\d*)\b(?! ?G4 ?P?\b)/g,
						`$& G4 P${spindleOnDelay}`,
					);
				}
			...
			"gcode:start": () => {
				const preferences = store.get("preferences", {});
				const defaultSpindleDelay = Number(
					_.get(preferences, "spindleDelay", 0),
				);
				...
				if (lineToStartFrom && lineToStartFrom <= totalLines) {
					...
					const { x: xVal, y: yVal, z: zVal, a: aVal } = position;
					...
					const setModalGcode =
						modal.motion === "G2" || modal.motion === "G3"
							? `${modal.motion} X${xVal.toFixed(3)} J0 F${feedRate}`
							: `${modal.motion}`;
					...
					modalGCode.push(
						`${modal.units} ${modal.distance} ${modal.arc} ${modalWcs} ${modal.plane} ${coolant.flood} ${coolant.mist}`,
					);
			...
			"lasertest:on": () => {
				...
				const laserPower = ensurePositiveNumber(maxS * (power / 100)).toFixed(
					2,
				);
				const commands = [
			...
			"laserpower:change": () => {
				...
				const commands = [
					...
					// The laser will only turn on when Grbl is in a G1, G2, or G3 motion mode.
					"S" +
						Math.round(ensurePositiveNumber(maxS * (power / 100)) * 100) / 100,
				];
				this.command("gcode", commands);
			},
			"spindlespeed:change": () => {
				const [speed = 0] = args;
				const commands = ["S" + speed];
				this.command("gcode", commands);
			},
			gcode: () => {
				const [commands, context] = args;
				...
				setTimeout(async () => {
					if (this.runner.isSDMounted()) {
						if (this.connection.isNetwork()) {
							...
							await this.ftpClient.openConnection(
								address,
								FTPPort,
								"grblHAL",
								"grblHAL",
	...
	setSenderTimeout() {
		...
		this.timeout = setTimeout(() => {
			...
			if (this.state.status.activeState === GRBL_HAL_ACTIVE_STATE_IDLE) {
				...
				this.sender.ack();
				this.runner.forceOK();
			} else if (this.workflow.state === WORKFLOW_STATE_RUNNING) {
				// if job not idle but running, reset timeout
				this.setSenderTimeout();
			} else if (
				this.state.status.activeState === GRBL_HAL_ACTIVE_STATE_IDLE &&
				this.workflow.state === WORKFLOW_STATE_IDLE
			) {
				// job done

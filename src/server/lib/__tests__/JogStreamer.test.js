...
describe("computeSegmentPlan", () => {
	...
	it("does not let a silent axis read as infinitely capable", () => {
		// Y is missing. It must still constrain motion using the current
		// fallback acceleration, rather than being treated as infinitely capable.
		...
		const known = computeSegmentPlan({
			dir: { X: 1, Y: 1 },
			feedrate: 3000,
			accelByAxis: { X: 500, Y: ASSUMED_ACCEL },
			maxRateByAxis,
		});

		expect(partial.effectiveAccel).toBeCloseTo(known.effectiveAccel);
		expect(partial.effectiveAccel).toBeLessThan(500 * Math.SQRT2);
...
describe("JogStreamer velocity mode", () => {
	...
	it("does not warn about acceleration the firmware never reported", () => {
		...
		// Exceed the queue depth for the current fallback acceleration.
		// An assumed limit must not produce a reported-machine warning.
		ctx.streamer.start({ axes: { X: 1 }, feedrate: ASSUMED_ACCEL * 60 });
